...
const Header = () => {
  ...
  return (<>
    <div className='h-20 w-full px-16 flex justify-between items-center fixed z-10' style={{"backgroundColor" : "rgb(0, 150, 150)"}}>
      <div className='text-2xl font-bold text-white whitespace-nowrap mr-7' style={{"textShadow": "0 4px 10px rgb(0 0 0 / 25%)"}}>INTEX-MARKET.UZ</div>
      <div className={`${styles.categoryScroll} flex overflow-x-auto items-center text-xl text-white`}>
        {
      ...
      <div className='flex justify-between items-center min-w-[270px] ml-7'>
        <Link href={`tel:${baseInfo.phone_number}`} className='hover:cursor-pointer'><a className='text-white text-md font-semibold'>{baseInfo.phone_number}</a></Link>
        <div className='flex items-center ml-5 justify-between w-28 hover:cursor-pointer'>
           <a href={baseInfo.telegram_link} className='inline-block mt-1.5' target="_blank" rel="noreferrer">
            <Image 
           ...
           <a href={baseInfo.instagram_link} className='inline-block mt-1.5' target="_blank" rel="noreferrer">
            <Image   
              className='hover:cursor-pointer'
              alt="telegram" 
          ...
          <div onClick={() => dispatch(changeLang(lang === "UZ" ? "RU" : "UZ"))} className='bg-white rounded-sm hover:cursor-pointer py-0.5 px-1 font-bold' style={{"width": "30px", "height": "29px", "color": " rgb(0, 150, 150)"}}>
            {lang}
          </div>
        </div> 
      </div>
