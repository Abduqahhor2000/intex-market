...
function Menu({setIsMenuOpen}) {
  ...
  return (
    <div className='flex flex-col h-full justify-between pb-4 min_sm:pb-12'>
        <div className='text-2xl font-semibold text-white text-center mt-5'>INTEX-SHOP.UZ</div>
        ...
        <div className='px-6'> 
            <div className='w-full shadow-xl rounded-xl text-center text-xl text-white font-bold mb-4 min_sm:mb-7 bg-green-recommend' >
              <a href={`tel:${baseInfo.phone_number}`} className="flex items-center py-1.5 min_sm:py-3.5 pl-10">
                <span className='mr-4'>{phone_icon}</span>
                <span className='mb-1'>{lang === "RU" ? "Позвонить" : "Qo'ng'iroq"}</span>
              </a>
            </div>  
            <div className='bg-white shadow-xl w-full rounded-xl text-center text-xl font-bold mb-4 min_sm:mb-7 text-green-main '>
              <a href={baseInfo.telegram_link} target="_blank" rel="noreferrer" className="flex items-center py-1.5 min_sm:py-3.5 pl-10">
                <span className='mr-4'>{telegram_icon}</span>
                <span className='mb-1'>{lang === "RU" ? "Телеграм" : "Telegram"}</span>
              </a>
            </div>  
            <div className='bg-white shadow-xl w-full rounded-xl text-center text-xl text-green-main font-bold'>
              <a href={baseInfo.instagram_link} target="_blank" rel="noreferrer" className="flex items-center py-0.5 min_sm:py-2 pl-9">
                <span className='mr-2.5 w-10 h-10'>{instagram_icon}</span>
                <span className='mb-1'>{lang === "RU" ? "Инстаграм" : "Instagram"}</span>
              </a>
            </div>  
        </div>                         
    </div>
